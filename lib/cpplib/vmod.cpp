    static Value vmod(const Value& val_1_v, const Value& val_2_v, const int line, const char* func){
        ...
        if (std::holds_alternative<long long>(val_1) && std::holds_alternative<long long>(val_2)) {
            ...
            long long raw_res = std::get<long long>(val_1) % static_cast<long long>(std::round(std::get<long double>(val_2)));
            return Value(raw_res);
        }
        else if (std::holds_alternative<long double>(val_1) && std::holds_alternative<long long>(val_2)) {
            long long raw_res = static_cast<long long>(std::round(std::get<long double>(val_1))) % std::get<long long>(val_2);
            return Value(raw_res);
        }
        else if (std::holds_alternative<long double>(val_1) && std::holds_alternative<long double>(val_2)) {
            long long raw_res = static_cast<long long>(std::round(std::get<long double>(val_1))) % static_cast<long long>(std::round(std::get<long double>(val_2)));
            return Value(raw_res);
        }
        else {
            RunTime instance;
            instance.throw_rt_error("Invalid type for operation: 'mod': " + get_dbg_type(val_1) + " % " + get_dbg_type(val_2), line, func);
            return Value(none{});
        }
    }
